...
<?php include dirname(__FILE__) . '/nav.php'; ?>
  ...
  <article>
    ...
    <section>    
      ...
      <ul id=bookmarklets>
  <?php foreach (array('m3u', 'rss', 'xspf') as $format): ?>
        <li><a href="javascript:location.href='<?php print PLAYER; ?>?url='+encodeURIComponent(location.href)+'&format=.<?php print $format; ?>'"><?php print strtoupper($format); ?></a>
  <?php endforeach; ?>
		    <li><a href="javascript:location.href='<?php print FLASH_PLAYER; ?>?repeat=list&autostart=true&skin=bekle.swf&playlist=bottom&playlistsize=400&playlistfile='+ encodeURIComponent('play.php?url='+encodeURIComponent(location.href)+'&format=.xspf')">Flash Audio</a>
      </ul>
     </section>
